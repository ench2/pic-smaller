...
function isHeif(item: ImageItem) {
  ...
  return (
    ["image/heic", "image/heif"].includes(item.blob.type.toLowerCase()) ||
    extension === "heic" ||
    extension === "heif"
  );
}
...
function IconButton({
  ...
  return (
    <button
      type="button"
      className={danger ? style.dangerButton : style.iconButton}
      aria-label={label}
      title={label}
      disabled={disabled}
      onClick={onClick}
...
const ResultItem = observer(
  ({
    ...
    const labels = getHomeCopy(gstate.lang).status;
    ...
    } else if (item.preservedOriginal) {
      statusIcon = <AlertTriangle size={17} className={style.preservedIcon} />;
    } else if (completed) {
    ...
    return (
      <article className={style.resultItem}>
        <button
          ...
          onClick={compare}
          disabled={!item.compress || Boolean(homeState.isCropMode())}
          aria-label={gstate.locale?.previewHelp}
        >
          {item.preview ? <img src={item.preview.src} alt="" /> : <span />}
          {item.compress && !homeState.isCropMode() && (
            <i>
              <Eye size={18} />
            </i>
          )}
        ...
        <div className={style.outputInfo}>
          ...
          <strong className={reduced ? style.success : style.warning}>
            {outputSize === undefined ? "-" : formatSize(outputSize)}
          </strong>
          <span>
            {item.compress
              ? `${item.compress.width} x ${item.compress.height}`
              : "-"}
          </span>
        </div>
        ...
        <div className={style.itemActions}>
          ...
          <IconButton
            label={gstate.locale?.listAction.removeOne ?? "Remove"}
            danger
            disabled={disabled}
            onClick={() => homeState.remove(item.key)}
          >
            <Trash2 size={18} />
          </IconButton>
        </div>
        {item.processError &&
          (() => {
            const errorText =
              item.processError === ERROR_ANIMATED_UNSUPPORTED
  ...
  return (
    <div className={style.content}>
      <div className={style.toolbar}>
        ...
        <div>
          <IconButton
            label={gstate.locale?.listAction.reCompress ?? "Recompress"}
            disabled={disabled}
            onClick={() => homeState.reCompress()}
          >
            <RefreshCw size={18} />
          </IconButton>
          <IconButton
            label={gstate.locale?.listAction.clear ?? "Clear"}
            danger
            disabled={disabled}
            onClick={() => homeState.clear()}
          >
            <Trash2 size={18} />
          </IconButton>
          <button
      ...
      <div className={style.list}>
        {takeItems(homeState.list.values(), visibleCount).map((item) => (
          <ResultItem
            key={item.key}
            item={item}
            disabled={disabled}
            onPreviewUnavailable={() =>
              setToast(gstate.locale?.heif.previewUnavailable ?? "")
            }
          />
        ))}
      </div>
      <footer ref={progressRef} className={style.progress}>
        <ProgressHint />
      </footer>
      {toast && (
